(function() {
	...
	function workGuide($http) {
		var url_workGuide =  "/contents/app/workGuide/data/inform.list.json";
		var url_back = '#/workGuide';
		var url_role = "/role";
		var service = {
			grid : grid,
			getworkGuideById : getworkGuideById,
			initZtreeClient : initZtreeClient,
			createworkGuide : createworkGuide,
			deleteworkGuide : deleteworkGuide,
			updateworkGuide : updateworkGuide
		};

		return service;

		// begin#updateworkGuide
		function updateworkGuide(vm) {
		...
		// begin#deleteworkGuide
		function deleteworkGuide(vm, id) {
			vm.isSubmit = true;
			var httpOptions = {
				method : 'delete',
				url : url_workGuide,
		...
		// begin#createworkGuide
		function createworkGuide(vm) {
			common.initJqValidation();
			var isValid = $('form').valid();
			if (isValid) {
				vm.isSubmit = true;
		...
		// begin#getworkGuideById
		function getworkGuideById(vm) {
			var httpOptions = {
				method : 'get',
				url : common.format(url_workGuide + "?$filter=id eq '{0}'", vm.id)
			}
			var httpSuccess = function success(response) {
