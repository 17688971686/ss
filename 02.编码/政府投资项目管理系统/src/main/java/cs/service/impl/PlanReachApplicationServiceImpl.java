...
//		shenBaoInfoDto.setApInvestSum(entity.getProjectInvestAccuSum());
    ...
    public void addPackPlan(String planReachId, String packPlanId) {
        ...
        loop:
        for (int i = 0; i < planReach.getPackPlans( ).size( ); i++) {
    ...
    public void startProcess(String packId) {
        ...
        for (int i = 0; i < shenbaoList.size( ); i++) {
            ...
            if (!StringUtils.isEmpty(entity.getThisTaskName( ))) {
    ...
    public void updateShnebaoInfo(String shenbaoId, Double ggmoney, Double gtmoney) {
        ...
        if (ObjectUtils.isEmpty(entity.getPackPlanId( ))) {
            ...
            if (Double.doubleToLongBits(ggmoney) >Double.doubleToLongBits( entity.getCapitalAP_ggys_TheYear())) {
                throw new IllegalArgumentException("超过年度安排资金-公共预算,无法提交！");
            }
            if (Double.doubleToLongBits(gtmoney) > Double.doubleToLongBits(entity.getCapitalAP_gtzj_TheYear( ))) {
                throw new IllegalArgumentException("超过年度安排资金-国土资金,无法提交！");
            }
            if (Double.doubleToLongBits(DoubleUtils.sum(ggmoney , gtmoney) + entity.getApplyAPYearInvest( )) > Double.doubleToLongBits(entity.getYearInvestApproval( ))) {
    ...
    public void deletePack(String planReachId, String packId) {
        ...
        //  不能删除，否则其他计划下达添加的申报信息也会被删掉，打包作为公共信息
    ...
    public void deletePlanShenBaoInfo(String packPlanId, String shenbaoId) {
        ...
        if(!ObjectUtils.isEmpty(entity)){
            //进行中或者已审批通过的项目无法删除
            if (!StringUtils.isEmpty(entity.getProcessState( ))&&!entity.getProcessState( ).equals(BasicDataConfig.processState_jinxingzhong) &&
                    !entity.getProcessState( ).equals(BasicDataConfig.processState_pass)) {

                if(!CollectionUtils.isEmpty(plan.getShenBaoInfos())){
            ...
            throw new IllegalArgumentException("项目查询失败！");
        }
    ...
    public void deleteProcessOne(String shenbaoId) {
        ...
        if ("投资科审核收件办理".equals(entity.getProcessStage( )) || "".equals(entity.getProcessStage( ))) {
            ...
            if(!CollectionUtils.isEmpty(shenBaoInfo)){
