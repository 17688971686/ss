...
//			         }
    ...
    public PageModelDto<PackPlanDto> getPackPlan(ODataObj odataObj) {
        ...
        //获取当前年
        int year = cal.get(Calendar.YEAR), nextYear = year + 1;
        ...
        // TODO 筛选出包含有本单位的编制
        if (null != yearPlan && !CollectionUtils.isEmpty(yearPlan.getPackPlans())) {
...
//
//            }
            List<PackPlan> oldPackPlans = yearPlan.getPackPlans();
            int len = oldPackPlans.size();
            PackPlan packPlan;
            for (int i = 0; i < len; i++) {
                packPlan = oldPackPlans.get(i);
                if (null != packPlan && !CollectionUtils.isEmpty(packPlan.getAllocationCapitals())) {
                    for (int j = 0; j < packPlan.getAllocationCapitals().size(); j++) {
                        if (packPlan.getAllocationCapitals().get(j) != null) {
                            if (packPlan.getAllocationCapitals().get(j).getUnitName().equals(userUnitInfoDto1.getId())) {//如果有本单位的打包计划
...
//								a += yearPlan.getPackPlans().get(i).getAllocationCapitals().get(j).getCapital_gtzj();
                ...
                if (!isOurUnit) {
                    oldPackPlans.remove(i);
                }
            }
            len = oldPackPlans.size();
            List<PackPlanDto> packPlanList = new ArrayList<>(len);
            PackPlanDto packPlanDto;
            for (int i = 0; i < len; i++) {
                packPlanDto = packPlanMapper.toDto(oldPackPlans.get(i));
                packPlanList.add(packPlanDto);
            }

            return new PageModelDto<>(packPlanList, packPlanList.size());
        }
        ...
        if (planReach != null && null != entity) {
            if (!entity.getIsInPlan()) {
                ...
                if (hasPackPlan) {
                    ...
                    double a = 0.0;
                    boolean isOurUnit = false;
                    if (!CollectionUtils.isEmpty(entity.getAllocationCapitals())) {
            ...
            if (array_element.getShenBaoInfos() != null) {
                for (int j = 0; j < array_element.getShenBaoInfos().size(); j++) {
                    ...
                    if (shenbaoInfo.getProcessState().equals(BasicDataConfig.processState_jinxingzhong)) {
                        throw new IllegalArgumentException(String.format("打包计划中存在审批中的项目【%s】,无法删除,请重新选着！", array_element.getName()));
                    }
                }
            }
