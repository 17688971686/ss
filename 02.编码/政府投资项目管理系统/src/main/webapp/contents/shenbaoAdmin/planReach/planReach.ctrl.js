(function () {
    ...
    function planReach($location, planReachSvc,$state,$scope,$sce) {
        ...
        function list(){
        	...
        	//检查已纳入年度计划的计划下达申请资金
        	vm.check_ggys=function(id,compared){
        		vm.checkNumber(compared,vm.formatNumber($("#ggys_"+id).val()));
        	};
			vm.check_gtzj=function(id,compared){
				vm.checkNumber(compared,vm.formatNumber($("#gtzj_"+id).val()));
        	};
        	//已纳入年度计划的计划下达确认
