...
function LoadFile() {
	try{
		var webObj=document.getElementById("WebOffice1");
		webObj.HttpInit();
		webObj.HttpAddPostString("name", "123");
		webObj.HttpAddPostFile("111", "d:\\111.doc");
		webObj.HttpAddPostCurrFile("file", "");
		webObj.HttpPost("http://localhost:8080/demo");
	}catch(e){
		alert("异常\r\nError:"+e+"\r\nError Code:"+e.number+"\r\nError Des:"+e.description);
	}
}
...
function addBookmark() {
		alert("向光标处加入名称为test的书签");
			var webObj=document.getElementById("WebOffice1");
		  webObj.SetFieldValue("test", "加入书签test", "::ADDMARK::");


}
function taohong(){
	var webObj=document.getElementById("WebOffice1");
	webObj.SetFieldValue("test", "北京点聚信息技术有限公司", "");
...
function SaveDoc(id) {
	try{
		...
		returnValue = webObj.HttpPost("http://127.0.0.1/savedoc.jsp");	// 判断上传是否成功
...
function MarkProdect(){
	...
	for(iIndex=1;iIndex<=markCount;iIndex++){
		var vName = objWeb.GetBookMarkInfo(iIndex,0 ); //获取书签名称
		var markStar=objWeb.GetBMPos(vName,0);//获取书签起始位置
	  var markEnd=objWeb.GetBMPos(vName,1);//获取书签结束位置
	  if(MarkList==""){
	...
	if(MarkList==""){
	  alert("word文档中没有书签，不用设置书签保护");
	}else{
	  objWeb.LocalProtect(MarkList,"ProtectDoc","LOCK_INCLUDE");//设置书签区域加保护，其它地方可以修改
	}	
}

function  SelectDocProdect(){
	var objWeb=document.getElementById("WebOffice1");
	objWeb.LocalProtect("","ProtectDoc","LOCK_INCLUDE");//设置书签区域加保护，其它地方可以修改
