                    ...
                    fn: function () {
                        ...
                        if (vm.model.shenBaoInfo.projectInvestmentType == common.basicDataConfig().projectInvestmentType_SH) {//社会投资
                            ...
                            var child = $linq(common.getBasicData())
                                .where(function (x) {
                                    return x.id == vm.model.shenBaoInfo.projectIndustry;
                                })
                                .toArray()[0];
                        ...
                        if (vm.model.shenBaoInfo.projectShenBaoStage == common.basicDataConfig().projectShenBaoStage_nextYearPlan) {
                            ...
                            vm.uploadType = [['KXXYJBG', '可行性研究报告'], ['CBSJYGS', '初步设计与概算'], ['ZJSQBG', '资金申请报告批复']];
                            vm.isYearPlan = true;
                        }
                    }
                });
        ...
        function grid_yearPlan_shenbaoInfoList(vm) {
            ...
            var columns = [
...

                {
                    ...
                    width: 150,
                    template: function (item) {
                        return common.format('<span style="text-overflow:ellipsis;width:10px;overflow:hidden;white-space:nowrap;" title="{0}">{0}</span>', item.yearConstructionContentShenBao || '');
                    },
                    filterable: false,
                    headerAttributes: {
                        "class": "table-header-cell",
